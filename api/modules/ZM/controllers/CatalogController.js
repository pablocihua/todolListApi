...
// Interfaces
var CatalogI    = require('../models/interfaces/CatalogInterface')();
...
var CatalogActions    = {
    saveCatalog: function( req, res ){
        ...
        // Get request params.
        var params       = req.body;
        var _validate    = CatalogActions.validaterequiredFields( params, CatalogI );
        ...
        if( _validate.isValid ){
            ...
            .then( ( data ) => {
                if( data.data.docs.length ){
                    res.status( _status ).send({
                        message: 'El cataloge no puede registrarse.',
                        u: params
                    });
                } else {
                    // Encripting password
                    bcrypt.hash( params.password, null, null, ( err, hash ) => {
                        ...
                        CatalogI    = CatalogActions.fillInterface( params, CatalogI );

                        var CatalogModel    = Models.Model.create( CatalogI );
                        // Save Catalog in database.
                        CatalogModel.save(( function( error ){
                            if( error ){
                                _status    = 500;
                                response.message    = 'Error al guardar el cataloge';
                            } else {
                                _status    = 200;
                                response.message    = 'Se ha registrado el cataloge';
                            }
            ...
            response.message    = 'Error al validar los datos del cataloge.';
            res.status( _status ).send( response );
        }
    },
    ...
    updateCatalog: function( req, res ){
        ...
        let newCatalogI    = CatalogI;

        let _validate    = CatalogActions.validaterequiredFields( body, newCatalogI );
        if( _validate.isValid ){
            var mangoQuery    = {
                "selector": {
                    "_id": { "$eq": params.id },
            ...
            .then( ( data ) => {
                ...
                if( _data.length ){
                    ...
                    CatalogI    = CatalogActions.fillInterface( body, _data );
                    couchNode
                    .update( dbName, CatalogI )
                    .then(( saved ) => {
                        if( saved.data.ok ){
                            response.message    = 'Se ha actualizado el cataloge';
                            response.Catalog       = CatalogI;
                        } else {
    ...
    uploadImage: function( req, res ){
        ...
        if( Object.keys( req.files ).length ){
            ...
            if( allowed.length ){
                couchNano.get( CatalogId, function( err, doc ){
                    _status    = 200;
                    if( err ){
                                ...
                                if( _err ){
                                    ...
                                    fs.readFile( file_path, ( err, attch ) => {
                                        if( err ){
                                            ...
                                            couchNano.get( CatalogId, function( err, _doc ){
                                                couchNano
                                                .attachment
                                                .insert( CatalogId, file_name, attch, contentType,
                                                    { rev: _doc._rev },
                                                    function( err, _body ){
                                                        if( err ){
                                                            _status    = 500;
    ...
    getCatalogs: function( req, res ){
        ...
        couchNano
        .view( 'catalogs', 'all?descending=true', _query, ( error, data ) => { // &startkey=["true","alive"]&endkey=["true","alive"]
            console.log( error )
            var items         = [],
                _totalPage    = 1;
    ...
    searchCatalog: function( req, res ){
        // Final response.
        var response    = {
